...
## All Components - Pre-Build 

![HP Z440 Workstation](Z4401MachineBeforeOpening.jpg)

Before anything was installed, all components were laid out to verify everything was accounted
for. The Z440 chassis is open on the right, with the GPU, NVMe adapter, SSDs, and ECC RAM
sticks staged and ready on the left.
...
![All components laid out before installation](Z4402AllComponentsPreBuild.jpg)
...
speed still matters - lower-clocked CPUs feel sluggish on tasks that cannot be parallelized.
...
of silent data corruption that can cause mysterious crashes and filesystem issues. It is
standard in enterprise gear for a reason.

**Why 2400MHz?** The E5-2690 v4 has a maximum supported memory speed of 2400MHz therefore
2400MHz sticks were sourced specifically to run at the CPU's rated maximum. Faster sticks
would simply downclock to 2400MHz, so there is no benefit to going higher.
...
with this cooler already installed which means there is headroom to expand to 128GB later
simply by adding four more sticks, without any additional hardware required. For now, the four 16GB DDR4 RAM sticks should be plenty for what I am planning to do with this machine.

![64GB ECC DDR4 RAM installed](Z4403ECCMemoryInstalled.jpg)
...
![Installing NVMe SSDs into the Z Turbo Quad Pro adapter](Z4407TurboZQuadInstallingSSDs.jpg)
...
![PCIe component spacing with RX 6600 XT and Z Turbo adapter](Z44018PCIEComponentSpacing.jpg)
...
![Machine state before NIC and 2TB SSD upgrade](Z44012MachineBeforeInstallingDualNic2TBSSD.jpg)

![Intel I226-V dual port NIC closeup](Z44015DualNicCloseup.jpg)

![Intel I226-V dual port NIC installed view A](Z44016DualNicInstalledA.jpg)

![Intel I226-V dual port NIC installed view B](Z44017DualNicInstalledB.jpg)
...
| High-capacity NAS HDD | Set up RAID configuration for archive storage -- waiting for prices to normalize |
...
- Workstation-class hardware (Z440, Z series Xeon) hits a sweet spot for home lab use with
...
- Physical fitment is a real constraint - the GTX 1070 Ti technically worked but was not
